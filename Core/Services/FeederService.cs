...
public class FeederService : IFeederService
{
    ...
    {
        _logger = logger;
        logger.LogDebug("Retrieving events for cutoffWeek: {cutoffWeek} plant: {plant}", cutoffWeek, plant);
        ...
        string plant)
    {
        ...
        try
        {
            ...
            var mapper = CreateCommonLibMapper();
            var mappedMessages = messages.Select(m => mapper.Map(m).Message).Where(m => m.Objects.Any()).ToList();
            _logger?.LogDebug("Sending {mappedMessagesCount} messages for cutoffWeek: {cutoffWeek} plant: {plant} to Alpha/Fam", mappedMessages.Count(), cutoffWeek, plant);
            await SendFamMessages(mappedMessages);
            messagesCount += mappedMessages.Count;
        }
        catch (Exception ex)
